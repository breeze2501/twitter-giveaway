...
function cachePath(tweetId) {
  return `./cache/${tweetId}/${Follower.name}`;
}
...
export class Follower {
  ...
  save() {
    const path = cachePath(this.userId);
  ...
  url() {
    return `https://api.twitter.com/2/users/${this.userId}/followers`;
  }
  ...
  async fetch() {
    ...
    try {
      ...
      console.log(JSON.stringify(this.resultThisTime));
      throw e;
    }
  }
